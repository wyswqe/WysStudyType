...
namespace Assets.Script.Utility
{
    ...
    {
        public virtual CsvNameType ItemCsvName
        {
            get { return 0; }
        }
    ...
    public class ReadTextAssetMgr : TSingleton<ReadTextAssetMgr>
    {
        ...
        public override void Init()
        {
            base.Init();
            Debug.Log("开始下载");
            for (int i = 0; i < (int)CsvNameType.Max; i++)
            {
                ConfigCSVFileName((CsvNameType)i);
            }
        }
        ...
        public void DownLoad(string fileName, Action<string, List<string[]>> callback)
        {
            if (callback != null)
            {
                callback(fileName, ReadCSVAssetMgr.ReadCSV(fileName));
            }
        }
    }
}
